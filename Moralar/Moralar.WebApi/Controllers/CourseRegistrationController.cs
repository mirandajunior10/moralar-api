...
namespace Moralar.WebApi.Controllers
{
    ...
    public class CourseRegistrationController : Controller
    {
        ...
        public CourseRegistrationController(IMapper mapper, ICourseRegistrationRepository courseRegistrationRepository, ICourseRepository courseRepository, IFamilyRepository familyRepository, IHttpContextAccessor httpContextAccessor, IUtilService utilService)
        {
            _mapper = mapper;
        ...
        {
            var response = new DtResult<CourseRegistrationViewModel>();
            try
            {
                ...
                response.Data = _mapper.Map<List<CourseRegistrationViewModel>>(retorno);
        ...
        public async Task<IActionResult> Detail([FromRoute] string id)
        {
            try
            {
                ...
                return Ok(Utilities.ReturnSuccess(data: _mapper.Map<CourseRegistrationViewModel>(entity)));
            }
            catch (Exception ex)
            {
        ...
        [ProducesResponseType(500)]
        [AllowAnonymous]
        public async Task<IActionResult> Register([FromBody] CourseRegistrationViewModel model)
        {
            ...
            try
            {
                ...
                var entity = _mapper.Map<Data.Entities.CourseRegistration>(model);
                var entityId = await _courseRegistrationRepository.CreateAsync(entity).ConfigureAwait(false);
